...
function projectloop(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    var projectdiv = `<div class="bg-gray-50 w-11/12 p-3 border-2 rounded-lg shadow-lg text-center mb-8">
    <div class="text-xl text-purple-600 font-medium">${data[i].name}</div>
    <div class="px-4">
      ${data[i].description}
    </div>
    <div class="flex items-center justify-between">
      <a href="https://www.github.com/arafin02/${data[i].github}"><img class="icon" src="/media/github.svg" alt=""></a>
      <a class="text-purple-500" href="${data[i].url}">View Site</a>
    </div>
  </div>`;
    project += projectdiv
  }
